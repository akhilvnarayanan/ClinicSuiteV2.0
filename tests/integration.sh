...
request -H 'Content-Type: application/json' -d '{"Name":"Removable Patient"}' "$base/api/patients" >/dev/null; removable_patient_id="$(sqlite3 "$DATA/data/clinic.db" "SELECT Id FROM Patients WHERE Name='Removable Patient';")"; request -X DELETE "$base/api/patients/$removable_patient_id" >/dev/null; [[ "$(sqlite3 "$DATA/data/clinic.db" "SELECT COUNT(*) FROM Patients WHERE Id=$removable_patient_id;")" == "0" ]]
...
request -H 'Content-Type: application/json' -d '{"Name":"Removable Service","Price":15}' "$base/api/services" >/dev/null; removable_service_id="$(sqlite3 "$DATA/data/clinic.db" "SELECT Id FROM Services WHERE Name='Removable Service';")"; request -X DELETE "$base/api/services/$removable_service_id" >/dev/null; [[ "$(sqlite3 "$DATA/data/clinic.db" "SELECT COUNT(*) FROM Services WHERE Id=$removable_service_id;")" == "0" ]]
...
[[ "$(status -X DELETE "$base/api/patients/$pid")" == 409 ]]
...
[[ "$(status -X DELETE "$base/api/services/$active_service_id")" == 409 ]]
