...
CREATE TABLE IF NOT EXISTS Doctors(Id INTEGER PRIMARY KEY AUTOINCREMENT,Name TEXT NOT NULL,Specialization TEXT,Phone TEXT,ConsultationFee REAL NOT NULL DEFAULT 0,Active INTEGER NOT NULL DEFAULT 1);
...
CREATE TABLE IF NOT EXISTS Documents(Id INTEGER PRIMARY KEY AUTOINCREMENT,PatientId INTEGER NOT NULL,VisitId INTEGER NOT NULL,FilePath TEXT NOT NULL,OriginalName TEXT,DocumentType TEXT NOT NULL DEFAULT 'Prescription',CreatedAt TEXT NOT NULL,FOREIGN KEY(PatientId) REFERENCES Patients(Id),FOREIGN KEY(VisitId) REFERENCES Visits(Id));
 CREATE TABLE IF NOT EXISTS ClinicSettings(Id INTEGER PRIMARY KEY CHECK(Id=1),ClinicName TEXT NOT NULL,Address TEXT,Phone TEXT,Email TEXT,Website TEXT,RegistrationNo TEXT,TaxNo TEXT,Currency TEXT NOT NULL DEFAULT 'SGD',LogoPath TEXT,Footer TEXT,ManualBackupPath TEXT,BackupPath TEXT,BackupSchedule TEXT NOT NULL DEFAULT 'Off',BackupTime TEXT NOT NULL DEFAULT '02:00',BackupDay TEXT NOT NULL DEFAULT 'Monday',LastScheduledBackup TEXT);";
    ...
    public object ClinicProfile() { using var c = C.CreateCommand(); c.CommandText = "SELECT ClinicName,LogoPath FROM ClinicSettings WHERE Id=1"; using var r = c.ExecuteReader(); r.Read(); return new { ClinicName = r.GetString(0), LogoPath = r.IsDBNull(1) ? "" : r.GetString(1) }; }
    ...
    public object Doctors() { using var c=C.CreateCommand(); c.CommandText="SELECT Id,Name,Specialization,Phone,ConsultationFee,Active FROM Doctors ORDER BY Name"; using var r=c.ExecuteReader(); var a=new List<object>(); while(r.Read()) a.Add(new {id=r.GetInt64(0),name=r.GetString(1),specialization=r.IsDBNull(2)?"":r.GetString(2),phone=r.IsDBNull(3)?"":r.GetString(3),consultationFee=r.GetDecimal(4),active=r.GetInt64(5)==1}); return a; }
    public void AddDoctor(DoctorRequest x) { using var c = C.CreateCommand(); c.CommandText = "INSERT INTO Doctors(Name,Specialization,Phone,ConsultationFee) VALUES($n,$s,$p,$f)"; c.Parameters.AddWithValue("$n", x.Name.Trim()); c.Parameters.AddWithValue("$s", x.Specialization?.Trim() ?? ""); c.Parameters.AddWithValue("$p", x.Phone?.Trim() ?? ""); c.Parameters.AddWithValue("$f", x.ConsultationFee); c.ExecuteNonQuery(); }
    public bool UpdateDoctor(int id, DoctorRequest x) { using var c = C.CreateCommand(); c.CommandText = "UPDATE Doctors SET Name=$n,Specialization=$s,Phone=$p,ConsultationFee=$f,Active=$a WHERE Id=$i"; c.Parameters.AddWithValue("$i", id); c.Parameters.AddWithValue("$n", x.Name.Trim()); c.Parameters.AddWithValue("$s", x.Specialization ?? ""); c.Parameters.AddWithValue("$p", x.Phone ?? ""); c.Parameters.AddWithValue("$f", x.ConsultationFee); c.Parameters.AddWithValue("$a", x.Active ? 1 : 0); return c.ExecuteNonQuery() == 1; }
    ...
    public ClinicSettings Settings() { using var c = C.CreateCommand(); c.CommandText = "SELECT ClinicName,Address,Phone,Email,Website,RegistrationNo,TaxNo,Currency,LogoPath,Footer,ManualBackupPath,BackupPath,BackupSchedule,BackupTime,BackupDay,LastScheduledBackup FROM ClinicSettings WHERE Id=1"; using var r = c.ExecuteReader(); r.Read(); return new ClinicSettings(r.GetString(0), r.GetString(1), r.GetString(2), r.GetString(3), r.GetString(4), r.GetString(5), r.GetString(6), r.GetString(7), r.GetString(8), r.GetString(9), r.IsDBNull(10) ? "" : r.GetString(10), r.IsDBNull(11) ? "" : r.GetString(11), r.GetString(12), r.GetString(13), r.GetString(14), r.IsDBNull(15) ? null : r.GetString(15)); }
