...
<div><span>Bill ID</span><b>{{bill.id}}</b></div><div><span>Date</span><b>{{bill.date}}</b></div><div><span>Patient ID</span><b>{{patient.code}}</b></div><div><span>Patient Name</span><b>{{patient.name}}</b></div>
</section>
...
""";
    ...
    public const string PreviousServiceBillInvoiceHtml = """
<header class="letterhead">{{clinic.logo}}<div class="clinic-copy"><h1 class="clinic-name">{{clinic.name}}</h1>{{clinic.type}}{{clinic.address}}{{clinic.phone}}{{clinic.email}}</div></header>
<hr class="rule">
<h1 class="service-bill-title">Service Bill</h1>
<section class="bill-info">
