...
CREATE TABLE IF NOT EXISTS Documents(Id INTEGER PRIMARY KEY AUTOINCREMENT,PatientId INTEGER NOT NULL,VisitId INTEGER NOT NULL,FilePath TEXT NOT NULL,OriginalName TEXT,DocumentType TEXT NOT NULL DEFAULT 'Prescription',CreatedAt TEXT NOT NULL,FOREIGN KEY(PatientId) REFERENCES Patients(Id),FOREIGN KEY(VisitId) REFERENCES Visits(Id));
 CREATE TABLE IF NOT EXISTS PatientDocuments(Id INTEGER PRIMARY KEY AUTOINCREMENT,PatientId INTEGER NOT NULL,FilePath TEXT NOT NULL,OriginalName TEXT,DocumentType TEXT NOT NULL DEFAULT 'Patient document',CreatedAt TEXT NOT NULL,FOREIGN KEY(PatientId) REFERENCES Patients(Id));
  ...
  CREATE TABLE IF NOT EXISTS DocumentTemplates(Id INTEGER PRIMARY KEY CHECK(Id=1),PrescriptionHtml TEXT NOT NULL,InvoiceHtml TEXT NOT NULL);";
        cmd.ExecuteNonQuery();
    ...
    public object AddPatient(PatientRequest x)
    {
        using var transaction = C.BeginTransaction();
        var temporaryCode = "TMP-" + Guid.NewGuid().ToString("N");
        using var insert = C.CreateCommand();
        insert.Transaction = transaction;
        insert.CommandText = "INSERT INTO Patients(PatientCode,Name,Phone,Email,Address,DateOfBirth,Gender,BloodGroup,Allergies,MedicalConditions,CreatedAt) VALUES($c,$n,$p,$e,$a,$d,$g,$b,$al,$mc,$t);SELECT last_insert_rowid();";
        foreach (var p in new[] { ("$c", (object)temporaryCode), ("$n", x.Name.Trim()), ("$p", x.Phone ?? ""), ("$e", x.Email ?? ""), ("$a", x.Address ?? ""), ("$d", x.DateOfBirth ?? ""), ("$g", x.Gender ?? ""), ("$b", x.BloodGroup ?? ""), ("$al", x.Allergies ?? ""), ("$mc", x.MedicalConditions ?? ""), ("$t", DateTime.Now.ToString("s")) })
            insert.Parameters.AddWithValue(p.Item1, p.Item2);
        var id = Convert.ToInt64(insert.ExecuteScalar());
        var code = $"P{id:D6}";
        using var update = C.CreateCommand();
        update.Transaction = transaction;
        update.CommandText = "UPDATE Patients SET PatientCode=$c WHERE Id=$i";
    ...
    public bool PatientHasHistory(int id) { using var c = C.CreateCommand(); c.CommandText = @"SELECT EXISTS(
        SELECT 1 FROM Visits WHERE PatientId=$i
        UNION ALL SELECT 1 FROM Documents WHERE PatientId=$i
        UNION ALL SELECT 1 FROM PatientDocuments WHERE PatientId=$i)"; c.Parameters.AddWithValue("$i", id); return Convert.ToInt64(c.ExecuteScalar()) == 1; }
    public bool DeletePatient(int id) { using var c = C.CreateCommand(); c.CommandText = "DELETE FROM Patients WHERE Id=$i"; c.Parameters.AddWithValue("$i", id); return c.ExecuteNonQuery() == 1; }
    ...
    public object Services(bool activeOnly = false)
    {
        using var c = C.CreateCommand();
        c.CommandText = activeOnly
            ? "SELECT Id,Name,Price,Active FROM Services WHERE Active=1 ORDER BY Name"
            : "SELECT Id,Name,Price,Active FROM Services ORDER BY Name";
        using var r = c.ExecuteReader();
        var a = new List<object>();
        while (r.Read())
            a.Add(new
    ...
    public bool ServiceHasCharges(int id) { using var c = C.CreateCommand(); c.CommandText = "SELECT EXISTS(SELECT 1 FROM VisitServices WHERE ServiceId=$i)"; c.Parameters.AddWithValue("$i", id); return Convert.ToInt64(c.ExecuteScalar()) == 1; }
    public bool DeleteService(int id) { using var c = C.CreateCommand(); c.CommandText = "DELETE FROM Services WHERE Id=$i"; c.Parameters.AddWithValue("$i", id); return c.ExecuteNonQuery() == 1; }
    ...
    {
        using var c = C.CreateCommand();
        c.CommandText = "SELECT PrescriptionHtml,InvoiceHtml FROM DocumentTemplates WHERE Id=1";
        using var r = c.ExecuteReader();
        if (!r.Read()) return new { prescriptionHtml = TemplateDefaults.PrescriptionHtml, invoiceHtml = TemplateDefaults.InvoiceHtml, defaultPrescriptionHtml = TemplateDefaults.PrescriptionHtml, defaultInvoiceHtml = TemplateDefaults.InvoiceHtml };
        return new { prescriptionHtml = r.GetString(0), invoiceHtml = r.GetString(1), defaultPrescriptionHtml = TemplateDefaults.PrescriptionHtml, defaultInvoiceHtml = TemplateDefaults.InvoiceHtml };
    }
    public void SaveTemplates(TemplateSettings x)
    {
        using var c = C.CreateCommand();
    ...
    public void AddDocument(int p, int v, string path, string original) { using var c = C.CreateCommand(); c.CommandText = "INSERT INTO Documents(PatientId,VisitId,FilePath,OriginalName,CreatedAt) VALUES($p,$v,$f,$o,$t)"; c.Parameters.AddWithValue("$p", p); c.Parameters.AddWithValue("$v", v); c.Parameters.AddWithValue("$f", path); c.Parameters.AddWithValue("$o", original); c.Parameters.AddWithValue("$t", DateTime.Now.ToString("s")); c.ExecuteNonQuery(); }
     public object AddVisit(VisitRequest x)
     {
         if (!DoctorAssignmentIsAvailable(x.DoctorId)) throw new InvalidOperationException("The selected doctor is not available today.");
         var visitDate = string.IsNullOrWhiteSpace(x.VisitDate)
             ? DateTime.Now
             : DateTime.TryParse(x.VisitDate, out var parsed)
                 ? parsed
                 : throw new InvalidOperationException("The visit date is invalid.");
         using var c=C.CreateCommand();
         c.CommandText="INSERT INTO Visits(PatientId,DoctorId,VisitDate,VisitType,Notes,ConsultationFee) VALUES($p,$d,$v,$ty,$n,$f);SELECT last_insert_rowid()";
         c.Parameters.AddWithValue("$p",x.PatientId);
         c.Parameters.AddWithValue("$d",x.DoctorId??(object)DBNull.Value);
         c.Parameters.AddWithValue("$v",visitDate.ToString("s"));
         c.Parameters.AddWithValue("$ty",string.IsNullOrWhiteSpace(x.VisitType)?"General":x.VisitType.Trim());
         c.Parameters.AddWithValue("$n",x.Notes??"");
         c.Parameters.AddWithValue("$f",x.ConsultationFee);
         return new { id=Convert.ToInt64(c.ExecuteScalar()), patientId=x.PatientId };
     }
    ...
    public object AddVisitService(int id,VisitServiceRequest x)
    {
        ...
        using var c=C.CreateCommand();
        c.CommandText="INSERT INTO VisitServices(VisitId,ServiceId,Description,Amount) VALUES($v,$s,$d,$a);SELECT last_insert_rowid()";
    ...
    {
        using var transaction = C.BeginTransaction();
        using (var balance = C.CreateCommand())
        {
            balance.Transaction = transaction;
            balance.CommandText = @"SELECT COALESCE((SELECT ConsultationFee FROM Visits WHERE Id=$v),0)
                                    + COALESCE((SELECT SUM(Amount) FROM VisitServices WHERE VisitId=$v),0)
                                    - COALESCE((SELECT SUM(Amount) FROM Payments WHERE VisitId=$v),0)";
            balance.Parameters.AddWithValue("$v", id);
            var outstanding = Convert.ToDecimal(balance.ExecuteScalar() ?? 0);
            if (x.Amount > outstanding)
                throw new InvalidOperationException($"Payment cannot exceed the outstanding balance of {outstanding:0.00}.");
        }
        using var c=C.CreateCommand();
        c.Transaction = transaction;
        c.CommandText="INSERT INTO Payments(VisitId,Amount,Method,PaidAt) VALUES($v,$a,$m,$t);SELECT last_insert_rowid()";
        c.Parameters.AddWithValue("$v",id);
        c.Parameters.AddWithValue("$a",x.Amount);
        c.Parameters.AddWithValue("$m",string.IsNullOrWhiteSpace(x.Method) ? "" : x.Method.Trim());
        c.Parameters.AddWithValue("$t",DateTime.Now.ToString("s"));
        var paymentId = Convert.ToInt64(c.ExecuteScalar());
        transaction.Commit();
        return new{id=paymentId,visitId=id,amount=x.Amount};
    }
    ...
    public object PaymentHistoryForVisit(int visitId)
    {
        using var c = C.CreateCommand();
        c.CommandText = @"SELECT pay.Id,pay.PaidAt,pay.Amount,
                                 COALESCE(NULLIF((SELECT group_concat(vs.Description, ', ')
    ...
    public object DailyReport(string? date)
    {
        var d=string.IsNullOrWhiteSpace(date)?DateTime.Now.ToString("yyyy-MM-dd"):date;
        using var c=C.CreateCommand();
        c.CommandText = @"WITH visit_totals AS (
                              SELECT v.Id,
                                     v.ConsultationFee + COALESCE((SELECT SUM(Amount) FROM VisitServices WHERE VisitId=v.Id),0) AS Total
                              FROM Visits v
                              WHERE date(v.VisitDate)=$d
                          )
                          SELECT
                              COUNT(*),
                              COALESCE(SUM(Total),0),
                              COALESCE((SELECT SUM(Amount) FROM Payments WHERE date(PaidAt)=$d),0),
                              COALESCE(SUM(Total - COALESCE((SELECT SUM(Amount) FROM Payments WHERE VisitId=visit_totals.Id),0)),0)
                          FROM visit_totals";
        c.Parameters.AddWithValue("$d",d);
        using var r=c.ExecuteReader();
        r.Read();
        return new{date=d,visits=r.GetInt64(0),billed=r.GetDecimal(1),collected=r.GetDecimal(2),balance=r.GetDecimal(3)};
    }
